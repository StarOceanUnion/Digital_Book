  ...
  while (ptFontOpr)
    {
      ...
      if (iError == 0)
        {
          /* some fonts which may be the ascii font or gbk font use ascii to encode,
           * so just need one FontInit() success, the func SetTextDetail can return success finally
           */
          iRet = 0;
        }
      else
  ...
  while (1)
    {
      ...
      /* some text need "\n" and "\r" together to express line feed,
       * so we just need to handle once if the file has these string
       */
      if (dwCode == '\n')
        {
          ...
          if (0 == iError)
            {
              ...
              if (RelocateFontPos(&tFontBitMap))
                {
                  /* The remaining LCD spaces are not enough to display this font */
                  return 0;
                }
              DBG_PRINTF("%s %s %d\n", __FILE__, __FUNCTION__, __LINE__);
              ...
              /* continue to get next font to display */
              break;
            }
